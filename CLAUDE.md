...
- `src/ffmpegAudioPlayer/` - decodes a file's audio track with a second bundled-ffmpeg process into an audify (RtAudio) output stream. playFrom respawns ffmpeg with input-side `-ss` (the video decoder's seek pattern), pause kills the decoder and clears the device queue, mute sets the device volume to zero, and position is the playFrom offset plus device frames actually played. Every failure resolves to silent playback, never a crash
